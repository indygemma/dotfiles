...
function! OpenRGAPDFNameAndPage()
  let l:part = getline('.')
  if l:part =~ "polar:"
    " This happens when polar:<polar id>:?<page num> is found
    let l:splitted = split(l:part, 'polar:')
    let l:polar_info = split(l:splitted[-1], ' ')[0]
    let l:polar_info_splitted = split(l:polar_info, ':')
    let l:polar_id = l:polar_info_splitted[0]

    if len(l:polar_info_splitted) == 2
      let l:pagenum = l:polar_info_splitted[1]
    else
      let l:pagenum = 0
    endif
    ...
    "echo l:polar_id
    "echo l:pagenum
    let l:file = "~/.polar/" . l:polar_id . "/state.json"

    "echo l:file
    redir => message
    silent execute ":!cat " . l:file . " | jq .docInfo.filename"
    redir END

    let l:filename = split(message, '\n')[2][1:-2]
    let l:filename = fnameescape( "~/.polar/stash/" . l:filename )
    let l:page = l:pagenum
  else
    " This is called when opening RGA search result item
    let l:splitted = split(l:part, ':')
    let l:filename = fnameescape(l:splitted[0])
    let l:page = split(l:splitted[1], ' ')[1]
  endif

  let l:cmd = "evince " . l:filename . " -i " . l:page
  echo l:cmd
  call jobstart( l:cmd ) " async so we can still modify the buffer
endfunction

function! StorePolarIDFromRGALine()
  let l:part = getline('.')

  if l:part =~ "polar:"

    "
    " We are in a "polar:<polar id> string
    "
    let l:splitted = split(l:part, 'polar:')
    let l:polar_info = split(l:splitted[-1], ' ')[0]
    let l:polar_info_splitted = split(l:polar_info, ':')
    let l:id = l:polar_info_splitted[0]
    ...
    "
    " We are in a rga search result list item
    "

    let l:splitted = split(l:part, ':')
    "echo l:splitted
    let l:filename = fnameescape(split(l:splitted[0], '/')[-1])
    let l:pagenum = split(l:splitted[1], ' ')[1]
    "echo l:filename
    "echo l:pagenum
    redir => message
    silent execute ":!cd ~/.polar/ && rg " . l:filename
    redir END

    "echo message

    let l:output = split(message, '\n')[2]

    "echo l:output

    let l:splitted_output = split(l:output, '/state.json:')

    " write out the id
    let l:id = l:splitted_output[0]
    "echo l:id
    "let @+ = l:id
    "
    "echo l:id
  ...
  " write the json to a tmp file
  let l:file = "~/.polar/" . l:id . "/state.json"
  "silent execute ":!rm " . l:file
  "silent execute 'redir > ' . l:file
  "silent echomsg l:json_out
  "silent! redir END
  "
  ...
  redir => message
  silent execute ":!cat " . l:file . " | jq .docInfo.title"
  redir END

  "echo message

  let l:title = split(message, '\n')[2]
  "echo l:title

  let l:output = "polar:" . l:id . ":" . l:pagenum . " " . l:title
...
":nnoremap gO :!evince <cfile> -i 
:nnoremap gO :call OpenRGAPDFNameAndPage()<CR>
:nnoremap gI :call StorePolarIDFromRGALine()<CR>
